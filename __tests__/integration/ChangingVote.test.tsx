...
import PollsApp from "@/app/components/PollsApp";
    ...
    options: [
      {
        id: "option-4",
        name: "Tobey Maguire",
        image: "tobey_maguire.png",
        votes: 55,
      },
      {
        id: "option-5",
        name: "Andrew Garfield",
        image: "andrew_garfield.png",
        votes: 40,
      },
      {
        id: "option-6",
        name: "Tom Holland",
        image: "tom_holland.png",
        votes: 55,
      },
    ],
  },
...
describe("Changing an existing vote", () => {
  it("moves the hero icon and recalculates percentages when the vote changes", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /vote for tobey maguire/i }),
    );
    ...
    expect(
      within(resultsBar).getByText(/tobey maguire/i),
    ).not.toHaveTextContent("🦸");

    const tomMeter = within(resultsBar).getByRole("meter", {
      name: "Tom Holland vote percentage",
    });
