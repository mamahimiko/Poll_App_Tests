...
describe("Voting flow", () => {
  it("reveals results with recalculated percentages after a vote is given", () => {
    ...
    const thorMeter = screen.getByRole("meter", {
      name: "Thor vote percentage",
    });
  ...
  it("keeps all displayed percentages summing to approximately 100% after a vote", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /vote for captain marvel/i }),
    );

    const meters = screen.getAllByRole("meter");

    const totalPercentage = meters.reduce(
      (sum, meter) => sum + Number(meter.getAttribute("aria-valuenow")),
      0,
    );
