...
describe("Calculate percentages", () => {
  ...
  it("calculates the correct percentage for a simple two options case", () => {
    ...
    expect(result.find((option) => option.id === "option-1")?.percentage).toBe(
      25,
    );
    expect(result.find((option) => option.id === "option-2")?.percentage).toBe(
      75,
    );
  });
  ...
  it("recalculates a higher percentage after an option receives an additional vote", () => {
    ...
    const lokiBefore = before.find(
      (option) => option.id === "option-2",
    )!.percentage;
    const lokiAfter = after.find(
      (option) => option.id === "option-2",
    )!.percentage;
